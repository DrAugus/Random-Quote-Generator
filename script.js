...
let quoteContainerTag = document.querySelector(".quoteContainer");
let quoteTag = document.querySelector(".quote");
let authorNameTag = document.querySelector(".authorName");
let getBtnTag = document.querySelector(".getBtn");
let index;
getBtnTag.addEventListener("click", () => {
    quoteTag.classList.remove("animate__animated", "animate__fadeIn")
    index = Math.floor(Math.random() * 21);
    console.log(index);
    window.requestAnimationFrame(function() {
        quoteTag.classList.add("animate__animated", "animate__fadeIn")
    });

    quoteTag.innerHTML = quotes[index].quote;
    authorNameTag.innerHTML = "- " + quotes[index].source;
